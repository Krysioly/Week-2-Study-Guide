...
def without_duplicates(words):
    ...
    for word in words:
        no_dupes_dict[word] = word

    no_dupes_list = list(no_dupes_dict.keys())
...
def get_sum_zero_pairs(numbers):
    ...
    for key in numbers_dict:
        for num in numbers:
            if key + num == 0 and [key, num] not in pairs_list:
                pairs_list.append([key, num])
...
def top_chars(phrase):
    ...
    for key in letter_dict:
        if letter_dict[key] > max_count:
            max_count = letter_dict[key]
